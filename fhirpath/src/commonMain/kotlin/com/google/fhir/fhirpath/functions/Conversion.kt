...
internal fun Collection<Any>.toDateTime(): Collection<FhirDateTime> {
  check(size <= 1) { "toDateTime() cannot be called on a collection with more than 1 item" }

  if (isEmpty()) return emptyList()

  return when (val value = single()) {
    is FhirDateTime -> listOf(value)
    is FhirDate -> listOf(FhirDateTime.fromString(value.toString())!!)
    is String ->
      try {
        listOf(FhirDateTime.fromString(value)!!)
      } catch (_: Exception) {
        emptyList()
      }
    else -> emptyList()
...
internal fun Collection<Any>.toQuantity(unit: String?): Collection<Quantity> {
  ...
  return when (val item = single()) {
    ...
    is Int ->
      listOf(
        Quantity(
          value = Decimal(value = item.toDouble()),
          unit = com.google.fhir.model.r4.String("1"),
        )
      )
    is Double ->
      listOf(Quantity(value = Decimal(value = item), unit = com.google.fhir.model.r4.String("1")))
    is Boolean ->
      listOf(
        Quantity(
          value = Decimal(value = if (item) 1.0 else 0.0),
          unit = com.google.fhir.model.r4.String("1"),
        )
      )
    is String -> {
      // Regex to parse a quantity string like "5.5 'mg'"
      val regex =
...
internal fun Collection<Any>.toTime(): Collection<LocalTime> {
  TODO("Implement toTime() function")
}
...
internal fun Collection<Any>.convertsToTime(): Collection<Boolean> {
  TODO("Implement convertsToTime() function")
}
