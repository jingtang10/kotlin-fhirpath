...
import com.google.fhir.fhirpath.codegen.schema.StructureDefinition.Kind
...
abstract class R4HelperGenerationTask : DefaultTask() {
  ...
  fun generateCode() {
    ...
    TypeExtensionFileSpecGenerator.generate(
        modelPackageName = modelPackageName,
        fhirPathPackageName = fhirPathPackageName,
        structureDefinitions =
          structureDefinitions
            .filter { it.kind == Kind.COMPLEX_TYPE || it.kind == Kind.RESOURCE }
            .filterNot { it.name == "Element" }
            .filterNot { it.name == "Resource" }
            .filterNot { it.name == "DomainResource" },
      )
